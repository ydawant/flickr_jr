...
Dir[APP_ROOT.join('app', 'uploaders', '*.rb')].each do |uploader_file|
  filename = File.basename(uploader_file).gsub('.rb', '')
  autoload ActiveSupport::Inflector.camelize(filename), uploader_file
end
